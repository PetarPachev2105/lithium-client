...
class LithiumRoom extends Component {
    ...
    componentDidMount() {
        ...
        this.virtualLithiumRoom = new VirtualLithiumRoom(this.token, this.clientId, this.lithiumRoomId);
        ...
        window.addEventListener('scroll', () => {
            ...
            const observer = new IntersectionObserver(
                ([entry]) => this.isFirstMessageInViewport(entry.isIntersecting), {rootMargin: '60px'}
            );
    ...
    receiveMessage = (message) => {
        ...
        const previousLastMessage = messages.find((message) => message.isLastMessage === true);
    ...
    isFirstMessageInViewport = this.debounce((value) => {
        if (value) {
            this.setState({
                ...
                isFirstMessageInViewportCount: this.state.isFirstMessageInViewportCount + 1,
    ...
    loadingOldMessages = () => {
        if (this.state.messages.length % 100 === 0) {
            const messages = this.state.messages;
            const firstMessage = messages.find((message) => message.isFirstMessage === true);
    ...
    loadOldMessages = (messages) => {
        ...
        newMessages = newMessages.sort((m1, m2) => (m1.number > m2.number) ? 1 : (m1.number < m2.number) ? -1 : 0);
        ...
        return (
            <HotKeys keyMap={keyMap} handlers={handlers} configure={configure}>
                <div>
                    ...
                    {!this.state.isInitialLoadingDone && <div>
                        Loading...
                    </div>}

                    {this.state.openedAddMemberMenu && <AddMemberMenu virtualLithiumRoom={this.virtualLithiumRoom}
                    ...
                    {this.state.openedChangeLithiumRoomNameMenu &&
                        <ChangeLithiumRoomNameMenu virtualLithiumRoom={this.virtualLithiumRoom}
                                                   closeChangeLithiumRoomNameMenu={this.closeChangeLithiumRoomNameMenu}
                                                   closeSettingsDialog={this.closeSettingsDialog}/>}
                    {this.state.openedManageMembersMenu &&
                        <ManageMembersMenu virtualLithiumRoom={this.virtualLithiumRoom}
                    ...
                    >
                        <div className={Classes.DIALOG_BODY}>
                            <button className={styles.largeButton} onClick={this.openAddMemberMenu}> Add Member</button>
                            <button className={styles.largeButton} onClick={this.openChangeLithiumRoomNameMenu}> Change
                                Lithium Room Name
                            </button>
                    ...
                    {this.state.isInitialLoadingDone && <section className={styles.lithiumRoom}>
                        ...
                        <main className={styles.lithiumChat} ref={this.lithiumRoomRef} onScroll={this.onScroll}>
                            {this.state.messages.map((message) => (
                                <div key={message.id}
                                    ...
                                    {message.isFirstMessage && <div className='first' ref={this.firstMessageRef}/>}
                                    <div
                                        className={styles.lithiumMessageImage}
                                    />
                                    <div className={styles.lithiumMessageBubble}>
                                        <div className={styles.lithiumMessageInfo}>
                                            <div className={styles.lithiumMessageInfoName}>{message.user.username}</div>
                                            <div
                                                className={styles.lithiumMessageInfoTime}>{message.sent_at_readable}</div>
                                        </div>

                                        <div
                                            className={message.user.username === this.state.username ? styles.lithiumMessageContentRight : styles.lithiumMessageContentLeft}>
                                            {message.content}
                                        </div>
                                        {message.isLastMessage && <div className='here' ref={this.lastMessageRef}/>}
                                    </div>
                                </div>
                            ))}
                        </main>
                        <FormGroup style={{'margin': '0'}} className={styles.lithiumRoomNewMessage}>
                            <input
                            ...
                            />
                            <button onClick={this.sendMessage} className={styles.lithiumRoomMessageSend}>Send</button>
                        </FormGroup>
                    </section>}
                </div>
            </HotKeys>
